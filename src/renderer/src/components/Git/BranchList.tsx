        ...
        {branches.map((branch) => (
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-sm font-medium text-text-primary truncate">{branch.branchName}</span>
            </div>
            <div className="text-xs text-text-secondary ml-4">{branch.taskTitle}</div>
            <div className="text-xs text-text-secondary ml-4 mt-1 flex items-center gap-2">
              <span>
                {branch.commitCount} commits &middot; {STATUS_LABELS[branch.status]}
                {branch.pushed && ' · Pushed'}
              </span>
              {branch.dirtyFileCount > 0 && (
                <span className="text-accent-amber text-[10px] ml-auto">{branch.dirtyFileCount} dirty</span>
              )}
            </div>
          </button>
        ))}
